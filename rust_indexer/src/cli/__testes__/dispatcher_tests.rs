...
mod tests {
    ...
    fn dispatch_list_languages_returns_true() {
        let line =
            r#"{"protocol_version":"1.0.0","type":"command","command":"list_languages","seq":1}"#;
    ...
    fn dispatch_unknown_returns_false() {
        let line =
            r#"{"protocol_version":"1.0.0","type":"command","command":"unknown_cmd","seq":2}"#;
        let handled = dispatcher::dispatch_line(line);
        assert!(
            !handled,
            "unknown commands should not be handled by dispatcher"
        );
    }
